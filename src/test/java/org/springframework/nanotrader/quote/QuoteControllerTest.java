...
public class QuoteControllerTest {
    ...
    @Test
    public void testResponseFndById() {
        String baseUri = "http://localhost:" + port + "/quotes";
        ResponseEntity<Quote> qr = restTemplate.getForEntity(baseUri + "/findBySymbol/YHOO", Quote.class);
        assertNotNull(qr);
        assertEquals(HttpStatus.OK, qr.getStatusCode());
        Quote q = qr.getBody();
        assertEquals("YHOO", q.getSymbol());

        qr = restTemplate.getForEntity(baseUri + "/findBySymbol/", Quote.class);
        assertNotNull(qr);
